...
function emojiToColor(emoji) {
  ...
  if (/🟢|💚|green/i.test(e))
    return "#7fb069";
  if (/🟡|💛|yellow|gold/i.test(e))
    return "#d9a340";
  if (/🟠|🟧|orange|peach/i.test(e))
    return "#e8935f";
  if (/🔴|❤️|red|pink/i.test(e))
    return "#e07a9c";
  if (/⚫|⚪|black|gray/i.test(e))
    return "#5a5660";
  return DEFAULT_DOT_COLOR;
}
function lunarLabelFor(y, m1, d) {
  try {
    const solar = Solar.fromYmd(y, m1 + 1, d);
    const lunar = solar.getLunar();
...
function lunarMonthLabelFor(y, m1) {
  try {
...
async function loadBloomDataLive(app) {
  ...
  if (!d.topTask) {
    ...
    if (first)
      d.topTask = { title: first.name, file: "11-Todo/Daily Tasks.md", done: false };
  }
  ...
  for (let d1 = 1; d1 <= daysInMonth; d1++) {
    ...
    if (hol)
      events.push({ day: d1, kind: "holiday", label: hol, color: "#b87b5a" });
    if (dayMd) {
...
var BloomView = class extends import_obsidian3.ItemView {
  constructor(leaf, settings) {
    ...
    __publicField(this, "calNav", { year: 2026, monthIndex: 7 });
  ...
  wire(root) {
    ...
    root.querySelectorAll("[data-cal-nav]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (next.year < 2026 || next.year > 2035)
          return;
        this.calNav = next;
        if (this.lastData)
          setCalendarMonth(this.containerEl, this.lastData, next);
      });
    });
  ...
  async toggleTopTask() {
    var _a, _b, _c;
    const card = this.containerEl.querySelector(".top-task-card");
    const btn = this.containerEl.querySelector("#top-task-check");
    if (!card || !btn)
      return;
    const file = card.dataset.file || TODO_FILE;
    const title = (_c = (_b = (_a = card.querySelector(".ttc-title")) == null ? void 0 : _a.textContent) == null ? void 0 : _b.trim()) != null ? _c : "";
    if (!title)
      return;
